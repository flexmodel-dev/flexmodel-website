...
| **Control Plane** | Service Auth   | 机器对机器访问（CI/CD、MCP 客户端、脚本调用） | 运维脚本、自动化集成      |
| **Control Plane** | Platform Auth  | 管理平台资源（创建项目、配置 IdP、管理用户/角色） | 管理后台、运维脚本       |
...
你能做什么？"）。Flexmodel 的 Federated Auth 层只做认证，不做授权决策——认证成功后返回 `userId`（身份标识）和 `scopes`
...
5. 提取 `sub`（用户标识）作为 `userId`，`scope` 作为 `scopes`
...
| 字段                    | 值                                                                    | 说明     |
|-----------------------|----------------------------------------------------------------------|--------|
| `userId`              | OIDC: `sub` 字段；Function: response body 中的 `userId` 或 `function-user` | 外部用户标识 |
...
| 字段                    | 值                    | 说明           |
|-----------------------|----------------------|--------------|
| `userId`              | API Key 的 `name`     | 密钥名称作为身份标识   |
| `projectId`           | URL 中的 `{projectId}` | 当前项目（需在白名单内） |
| `projectDatabaseName` | 项目对应的数据库名            | 用于数据隔离       |

## 第三层：Platform Auth（平台认证）
...
| 字段                    | Federated Auth              | Service Auth   | Platform Auth |
|-----------------------|-----------------------------|----------------|---------------|
| `projectId`           | URL 路径参数                    | URL 路径参数       | URL 路径参数      |
| `projectDatabaseName` | 项目数据库名                      | 项目数据库名         | 项目数据库名        |
| `userId`              | IdP `sub` 或 `function-user` | API Key `name` | JWT `account` |
